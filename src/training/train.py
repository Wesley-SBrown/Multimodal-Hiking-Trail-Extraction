...
def calculate_metrics(preds, targets):
    """
    Returns dice, precision, and recall for the trail class (label = 1).
    All three share the same TP/FP/FN computation so this is one pass.
    """
...
def run_smoke_test(model, train_loader, criterion_ce, criterion_dice, device):
    ...
    to verify that dataset, model, and loss functions work before full training loop begins
    """
...
def train_model():
    ...
    # metric history — one value appended per epoch
    history = {
        "train_loss": [],
        "val_loss":   [],
        "iou":        [],
    ...
    for epoch in range(1, epochs + 1):
        ...
        for batch_idx, (visual, elev, targets) in loop:
            ...
            # scaled gradients backward pass
        ...
        with torch.no_grad():
            for visual, elev, targets in val_loader:
                ...
                d, p, r = calculate_metrics(preds, targets)
                val_dice      += d
                val_precision += p
                val_recall    += r
        ...
        history["dice"].append(avg_val_dice)
        history["precision"].append(avg_val_precision)
        history["recall"].append(avg_val_recall)
        ...
        print(
            ...
            f"Dice: {avg_val_dice:.4f} | Precision: {avg_val_precision:.4f} | Recall: {avg_val_recall:.4f}"
        )
 
        # save checkpoint on best validation IoU
    ...
    # ── plot training diagnostics after all epochs complete ───────────────────
    print("\nGenerating training metrics plot...")
    metrics_path = os.path.join(PROJECT_ROOT, f"data/{region}_training_metrics.png")
    plot_training_metrics(history, save_path=metrics_path, region=region)
    plt.show()
